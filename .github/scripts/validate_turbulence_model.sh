...
if grep -q "nan\|inf\|-nan\|-inf" "$FINAL_VTK"; then
    echo "  ✗ FAILED: NaN or Inf detected in output!"
    VALIDATION_FAILED=1
else
...
if [ -z "$MAX_VEL" ] || [ "$MAX_VEL" = "0" ]; then
    echo "  ✗ FAILED: Zero velocity field (flow did not develop)!"
    VALIDATION_FAILED=1
else
...
if [ "$ZERO_COUNT" = "$TOTAL_COUNT" ]; then
    echo "  ✗ FAILED: All velocities are zero!"
    VALIDATION_FAILED=1
elif [ "$(echo "$ZERO_FRACTION > 0.5" | bc -l 2>/dev/null || echo 0)" = "1" ]; then
    echo "  ✗ WARNING: >50% of velocities are zero!"
    VALIDATION_FAILED=1
else
    echo "  ✓ Velocity field properly developed ($ZERO_COUNT / $TOTAL_COUNT cells are zero)"
fi
...
if grep -q "SCALARS nu_t" "$FINAL_VTK"; then
    ...
    if [ "$NEG_COUNT" -gt 0 ]; then
        echo "  ✗ FAILED: Negative eddy viscosity detected ($NEG_COUNT cells)!"
        VALIDATION_FAILED=1
    else
        echo "  ✓ All nu_t values are non-negative"
    fi
    
    # Check for NaN in nu_t specifically
    NAN_COUNT=$(echo "$NU_T_VALUES" | grep -c "nan\|inf" || true)
    if [ "$NAN_COUNT" -gt 0 ]; then
        echo "  ✗ FAILED: NaN/Inf in nu_t ($NAN_COUNT values)!"
        VALIDATION_FAILED=1
    else
        echo "  ✓ No NaN/Inf in nu_t"
    fi
    ...
    if [ "$IS_NU_T_REASONABLE" = "1" ]; then
        echo "  ✓ Eddy viscosity in reasonable range"
    else
...
if [ "$PRESSURE_NAN" -gt 0 ]; then
    echo "  ✗ FAILED: NaN/Inf in pressure field!"
    VALIDATION_FAILED=1
else
    echo "  ✓ Pressure field is finite"
fi
...
if [ -f "${OUTPUT_DIR}/channel_velocity.dat" ]; then
    ...
    if grep -q "nan\|inf" "${OUTPUT_DIR}/channel_velocity.dat"; then
        echo "  ✗ FAILED: NaN/Inf in velocity data file!"
        VALIDATION_FAILED=1
    else
        echo "  ✓ Velocity data file valid"
    fi
    
    if [ -f "${OUTPUT_DIR}/channel_nu_t.dat" ]; then
        if grep -q "nan\|inf" "${OUTPUT_DIR}/channel_nu_t.dat"; then
            echo "  ✗ FAILED: NaN/Inf in nu_t data file!"
            VALIDATION_FAILED=1
        else
            echo "  ✓ nu_t data file valid"
        fi
    fi
fi
...
if [ $VALIDATION_FAILED -eq 0 ]; then
    echo "✓ VALIDATION PASSED for $MODEL_NAME"
    echo "========================================"
    exit 0
else
