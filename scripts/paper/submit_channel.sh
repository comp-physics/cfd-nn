#!/bin/bash
# Submit all channel flow experiments: 5 models x 4 grids = 20 jobs
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
BUILD_DIR="$ROOT_DIR/build_paper"
RESULTS_BASE="$ROOT_DIR/results/paper/channel"
...
# NN preset flags for channel
declare -A NN_FLAGS
NN_FLAGS[baseline]=""
NN_FLAGS[sst]=""
NN_FLAGS[earsm_pope]=""
NN_FLAGS[nn_mlp]="--nn_preset mlp_channel_caseholdout"
NN_FLAGS[nn_tbnn]="--nn_preset tbnn_channel_caseholdout"
...
for model in "${MODELS[@]}"; do
    for grid in "${GRIDS[@]}"; do
        ...
        jobname="nn-paper-channel-${model}-${grid}"
        config="$CONFIG_DIR/channel_${grid}.cfg"
...
echo ""
echo "Total channel jobs submitted: ${submitted}"
