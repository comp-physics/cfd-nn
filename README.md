...
- **Skew-symmetric**: Energy-conserving split form, recommended for DNS/LES
- **First-order upwind**: More stable at high Reynolds numbers, increased numerical dissipation
- **Second-order upwind**: Less dissipative than first-order upwind
...
The solver supports **15 turbulence closure options** (RANS, LES, and neural network):
...
| `smagorinsky` | LES SGS | 0 | No | Yes |
| `dynamic_smagorinsky` | LES SGS | 0 | No | Yes |
| `wale` | LES SGS | 0 | No | Yes |
| `vreman` | LES SGS | 0 | No | Yes |
| `sigma` | LES SGS | 0 | No | Yes |
...
Five SGS models for Large Eddy Simulation, all GPU-accelerated via fused kernels that compute velocity gradients and eddy viscosity in a single pass:
...
**Architecture:** 6 → 32 → 32 → 1 (Tanh hidden activations, linear output)
...
**Architecture:** 5 → 64 → 64 → 64 → 4 (outputs coefficients for the 4 independent 2D basis tensors)
...
### IBM Flow Cases (Immersed Boundary Method)

Bluff body and complex geometry flows using direct-forcing IBM with GPU-accelerated pre-computed weight arrays:

| Configuration | Body | Executable | Use Case |
|---------------|------|------------|----------|
| Cylinder Re=100 | `CylinderBody` | `./cylinder` | Drag validation, vortex shedding |
| NACA Airfoil Re=1000 | `NACABody` | `./airfoil` | Lift/drag, pressure distribution |
| Periodic Hills Re=10595 | `PeriodicHillBody` | `./hills` | Separation/reattachment |
| Backward-Facing Step | `StepBody` | `./step` | Recirculation zone |

IBM supports 5 body types (cylinder, sphere, NACA 4-digit, backward step, periodic hills) with signed-distance-based cell classification. All IBM forcing runs on GPU via element-wise weight multiplication (<0.3% overhead).
...
| `smagorinsky` | LES: Static Smagorinsky SGS model |
| `dynamic_smagorinsky` | LES: Dynamic Smagorinsky (Germano) |
| `wale` | LES: Wall-Adapting Local Eddy-viscosity |
| `vreman` | LES: Vreman SGS model |
| `sigma` | LES: Sigma SGS model |
...
- Pressure Poisson solver (multigrid V-cycles, FFT, or HYPRE PFMG)
- Turbulence transport equations (k, ω) including wall BCs
- LES SGS models (fused gradient + eddy viscosity kernels)
- EARSM tensor basis computations
- Neural network inference (batched layer-by-layer parallelization)
- IBM direct forcing (pre-computed weight arrays, Poisson RHS masking)
...
- **Build with compute capability**: Use `cmake .. -DGPU_CC=90` for H100/H200, `-DGPU_CC=80` for A100, or comma-separated for multi-arch: `-DGPU_CC=70,80,89,90`.
...
### Turbulence Models (15 closures)

All 15 turbulence models (RANS + LES + NN) validated for stability on stretched grids:
...
| LES SGS | Smagorinsky, WALE, Vreman, Sigma, DynamicSmag | Stable, GPU-accelerated |
