...
Converting our H100 result to the same metric yields 1.59~ns/step/point at 18.9M cells for the base solver (no turbulence model).
The $2.2\times$ gap relative to CaLES's DNS mode is primarily attributable to GPU saturation: at 18.9M cells, the H100's 132 streaming multiprocessors are not fully occupied, as confirmed by the throughput increase from 433~Mcells/s at 2.1M cells to 626~Mcells/s at 18.9M cells (Table~\ref{tab:solver_throughput_grids}).
At CaLES's benchmark grid size (283M cells), our solver's per-point cost would decrease further as the GPU becomes fully saturated.
Normalizing by memory bandwidth (H100: 3.35~TB/s, A100: 2.0~TB/s) accounts for another factor, bringing the codes to within a factor of $\sim$$1.5\times$ at matched conditions.
