...
\toprule
GPU & Grid & Cells & ms/step & Mcells/s & ns/point \\
\midrule
\multirow{3}{*}{V100-PCIe-16GB}
 & $256 \times 128 \times 72$  & 2.4M  & 13.6  & 174 & 5.74 \\
 & $256^3$                     & 16.8M & 140.6 & 119 & 8.38 \\
 & $512 \times 256 \times 144$ & 18.9M & 146.4 & 129 & 7.76 \\
\midrule
\multirow{5}{*}{H100-SXM5-80GB}
 & $128^3$                     & 2.1M  & 5.44  & 385 & 2.60 \\
 & $256^3$                     & 16.8M & 27.3  & 616 & 1.62 \\
 & $512 \times 256 \times 144$ & 18.9M & 30.3  & 623 & 1.61 \\
 & $384 \times 256^2$          & 25.2M & 40.2  & 626 & 1.60 \\
 & $512 \times 256^2$          & 33.6M & 54.4  & 616 & 1.62 \\
\bottomrule
\end{tabular}
\end{table}

On the H100, throughput plateaus at $\sim$620--626~Mcells/s ($\sim$1.60~ns/step/point) for grids of 16.8M cells and above, indicating full GPU saturation.
The V100 peaks at $\sim$174~Mcells/s at 2.4M cells but decreases at larger grids, likely due to the 16~GB memory limit forcing larger FFT workspace allocations to spill.
...
As an additional comparison point, CaLES \citep{Xiao2024}---a large-eddy simulation code built on CaNS---reports a per-subroutine timing breakdown for its base solver (DNS mode, no turbulence model) on a single A100-64GB GPU with a $512 \times 384 \times 1440$ grid (283M cells):
\begin{itemize}
    \item Poisson solver (FFT): 38\%
    \item RHS (convection + diffusion): 30\%
    \item Velocity correction: 13\%
    \item Subgrid-scale model (LES Smagorinsky): 19\%
\end{itemize}
Their DNS (no SGS model) throughput on a single A100 is 1.5~ns/step/point, or equivalently $\sim$667~Mcells/s.
Note that CaLES is an LES code; their SGS model overhead (19\% for Smagorinsky) is not directly comparable to the RANS closure costs reported in Section~\ref{sec:cost}, as LES SGS models perform fundamentally different and less expensive operations (local eddy viscosity from resolved strain rate) compared to RANS closures (transport equations for $k$ and $\omega$, or neural network inference).

Our H100 achieves 1.60~ns/step/point at saturation (grids $\geq$16.8M cells; see Table~\ref{tab:solver_throughput_grids}), compared to CaLES's 1.5~ns/step/point on a single A100 at 283M cells.
This places our solver within 7\% of CaLES/CaNS in per-point throughput at GPU saturation, despite the architectural differences (OpenMP target offload vs.\ CUDA Fortran, C++ vs.\ Fortran).
The remaining gap is within the expected variation from memory bandwidth differences (H100: 3.35~TB/s, A100-SXM4: 2.0~TB/s---our H100 should be \emph{faster} per point, suggesting minor algorithmic differences or compiler-generated code quality account for the residual).
