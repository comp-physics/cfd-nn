...
- [x] **KNOWN LIMITATION: EARSM diverges on 3D duct** (investigated Mar 31)
  - EARSM-WJ/GS/Pope all NaN at step ~155 on 96³ duct, regardless of warmup time
  - NOT a warm-up toggle issue — diverges from cold start too (tested warmup=0/1/2/5/10s)
  - Root cause: EARSM G-coefficients produce locally negative effective viscosity near duct corners → explicit RK3 unstable. Literature uses implicit SIMPLE for RANS EARSM.
  - SST, RSM, MLP, TBNN, TBRF all stable on same config
  - EARSM works on 2D hills and cylinder (no corners)
