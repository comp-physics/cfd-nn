...
// Note: compute_u_profile is provided by test_utilities.hpp
...
// ============================================================================
// Parameterized RANS Channel Sanity Test
// ============================================================================

struct ChannelSanityParams {
...
static void test_rans_channel_generic(const ChannelSanityParams& p) {
    std::cout << "\n--- RANS Channel Sanity: " << p.prefix << " ---\n\n";
    ...
    config.tol = 1e-6;
    config.turb_model = p.type;
    config.verbose = false;

    RANSSolver solver(mesh, config);

    auto turb_model = create_turbulence_model(p.type, "", "");
    ...
    for (int iter = 0; iter < max_iters; ++iter) {
        ...
        if (p.warmup_iters > 0 && iter >= p.warmup_iters && iter % 50 == 0) {
            solver.sync_from_gpu();
            auto U_prof = compute_u_profile(solver, mesh);
            double Ub = compute_bulk_velocity(U_prof, Ly);
            U_bulk_history.push_back(Ub);
        }
    }
    solver.sync_from_gpu();
    ...
    record(p.prefix + " No-slip (|U_wall| < 30% of U_max)", no_slip_ok, ss.str());
    ss.str(""); ss << std::fixed << std::setprecision(1) << "(center=" << U_center/U_max*100 << "%, thr=50%)";
    record(p.prefix + " Centerline max (U_center >= 50% of U_max)", centerline_max_ok, ss.str());
    record(p.prefix + " Monotonic wall->center", monotonic_ok);
    record(p.prefix + " min(nu_t) >= -1e-12", min_nu_t_ok, qoi(nu_t_stats.min_nu_t, -1e-12));
    ss.str(""); ss << std::scientific << std::setprecision(2) << "(val=" << nu_t_stats.max_nu_t/nu << ", thr=1e6)";
    record(p.prefix + " max(nu_t)/nu < 1e6", max_nu_t_ok, ss.str());
    ss.str(""); ss << std::fixed << std::setprecision(2) << "(val=" << nu_t_stats.first_cell_nu_t/nu << ", thr=10)";
    record(p.prefix + " first-cell nu_t/nu < 10", first_cell_ok, ss.str());
    ss.str(""); ss << std::scientific << std::setprecision(2) << "(val=" << U_bulk << ")";
    record(p.prefix + " U_bulk > 0", U_bulk > 0, ss.str());
    record(p.prefix + " U_bulk stable (< 5% change)", U_bulk_stable);
    ss.str(""); ss << std::scientific << std::setprecision(2) << "(tau_w=" << tau_w << ")";
    record(p.prefix + " Wall shear sign correct", shear_sign_ok && shear_nonzero, ss.str());

    // Physics directionality checks
    record(p.prefix + " Production >= 0 in shear regions", production_positive);
    record(p.prefix + " Dissipation >= 0 everywhere", dissipation_positive);
    record(p.prefix + " TKE >= 0 everywhere", tke_nonnegative);
    if (p.emit_qoi) {
        record(p.prefix + " U_bulk growing toward steady", u_bulk_growing);
        harness::emit_qoi_rans_channel(U_bulk, nu_t_stats.max_nu_t / nu);
    }
}

void test_rans_channel_sst()    { test_rans_channel_generic({TurbulenceModelType::SSTKOmega, "SST", 200, true}); }
void test_rans_channel_komega() { test_rans_channel_generic({TurbulenceModelType::KOmega, "k-omega", 0, false}); }
